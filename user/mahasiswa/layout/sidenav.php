...
<ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
    ...
    if (str_contains($_SERVER['PHP_SELF'], '/dosen')) {
        ...
        if (str_contains($_SERVER['PHP_SELF'], '/dosen')) {
            ...
            <div class="bg-white py-2 collapse-inner rounded">
                ...
                <a class="<?= str_contains($_SERVER['PHP_SELF'], '/dosen') ? 'collapse-item active' : 'collapse-item' ?>"
                    href="../dosen/index.php">Dosen</a>
                <a class="<?= str_contains($_SERVER['PHP_SELF'], '/ormawa') ? 'collapse-item active' : 'collapse-item' ?>"
                    href="../ormawa/index.php">Ormawa</a>
                <a class="<?= str_contains($_SERVER['PHP_SELF'], '/mahasiswa') ? 'collapse-item active' : 'collapse-item' ?>"
                    href="../mahasiswa/index.php">Mahasiswa</a>
            </div>
        </div>
    </li>
    ...
    <li class="<?= str_contains($_SERVER['PHP_SELF'], '/ruangan') ? 'nav-item active' : 'nav-item' ?>">
        <a class="nav-link collapsed" href="#" data-toggle="collapse" aria-expanded="true"
            aria-controls="collapseRuangan" data-target="#collapseRuangan">
            <i class="fas fa-fw fa-door-open"></i>
            <span>Ruangan</span>
        </a>
        <div id="collapseRuangan"
    ...
    <li class="<?= str_contains($_SERVER['PHP_SELF'], '/peminjaman') ? 'nav-item active' : 'nav-item' ?>">
        ...
        <div id="collapsePeminjaman"
            class="<?= str_contains($_SERVER['PHP_SELF'], '/peminjaman') ? 'collapse show' : 'collapse' ?>"
            aria-labelledby="headingPages" data-parent="#accordionSidebar">
            <div class="bg-white py-2 collapse-inner rounded">
                <a class="<?= str_contains($_SERVER['PHP_SELF'], '/peminjaman/index.php') ? 'collapse-item active' : 'collapse-item' ?>"
                    href="../peminjaman/index.php">List</a>
            </div>
        </div>
    </li>
